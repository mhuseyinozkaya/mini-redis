...
#define UTILS_H
    ...
    #define DEBUG_SILENT(fmt, ...) do {fprintf(stdout,fmt, ##__VA_ARGS__); fflush(stdout);} while(0)
    #define DEBUG_LOG(fmt, ...) do {fprintf(stdout,"[DEBUG] " fmt, ##__VA_ARGS__); fflush(stdout);} while(0)
#else
